            ...
            for pos_i, test_idx in enumerate(test_indices):
                ...
                for trial in range(n_trials):
                    try:
                        ...
                        # Measure against the REQUESTED position. 'actual_position_mm'
                        # is a legacy alias for the SNAPPED position and measures a
                        # different quantity -- the defect WP-0 exposed. This call
                        # site arrived with the merge (run_noise_type_test is new on
                        # feat/validation-expectation-ranges) and so never received
                        # the v0.3.1 fix applied to the other three sites here.
                        true_pos = metadata['requested_position_mm']
                        error = self._find_peak_and_error(source_activity, true_pos)
